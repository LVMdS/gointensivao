...
func Test_If_It_Gets_An_Error_If_ID_Is_Blanck(t *testing.T) {
	order := Order{}
	assert.Error(t, order.Validate(), "invalid id")
}
func Test_If_It_Gets_An_Error_If_Price_Is_Blanck(t *testing.T) {
	order := Order{ID: "123"}
	assert.Error(t, order.Validate(), "invalid price")
}
func Test_If_It_Gets_An_Error_If_Tax_Is_Blanck(t *testing.T) {
